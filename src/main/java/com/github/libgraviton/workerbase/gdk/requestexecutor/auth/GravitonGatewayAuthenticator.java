...
public class GravitonGatewayAuthenticator implements Authenticator {
  ...
  public Request onRequest(Request request) throws AuthenticatorException {
    if (accessToken == null) {
      try {
        ...
        LOG.error(
          "Unable to get a gateway token, even after all retries. Giving up!",
          e
        );
  ...
  private void login() throws AuthenticatorException {
    ...
    if (cachedToken != null) {
      LOG.info("Using in a *cached* jwt token for user '{}'", gatewayUser);
    ...
    LOG.info(
      "Successfully received token and cached it for use of max '{}' minutes.",
